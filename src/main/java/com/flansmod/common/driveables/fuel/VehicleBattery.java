...
public class VehicleBattery extends InternalFuelTank {
	 ...
	 * @return 			A vehicle battery with its max energy capacity set to {@code maxEnergy}
	 */
	 ...
	 * @param maxReceive	The amount that can be received (RF/FE).
	...
	public int receiveEnergy(int maxReceive, boolean simulate) {
		int energyReceived = MathHelper.clamp(Math.min(FlansConfig.vehicles.energyTransferRate, maxReceive), 0, maxEnergy - energy);
        if (!simulate)
	...
	public ItemStack handleFuelItem(@Nonnull ItemStack stack) {
		...
		if (stack.hasCapability(CapabilityEnergy.ENERGY, null)) {
			...
			if (energyItem != null && energyItem.canExtract()) {
				//Check to see how much we can extract
				int amountToDrain = energyItem.extractEnergy(FlansConfig.vehicles.energyTransferRate, true);
